...
def greedy_coreset(
    ...
    with torch.no_grad():
        ...
        reduced = mapper(features_t)                           

    start_idx = np.random.choice(N, min(n_starting_points, N), replace=False).tolist()
    with torch.no_grad():
        anchors = reduced[start_idx]                           
        a2      = (reduced ** 2).sum(dim=1, keepdim=True)     
        b2      = (anchors ** 2).sum(dim=1, keepdim=True).T  
        ab      = reduced @ anchors.T                         
        dist_to_anchors = (a2 + b2 - 2 * ab).clamp(min=0).sqrt()

    approx_dists = dist_to_anchors.mean(dim=1, keepdim=True)  
    selected     = []
